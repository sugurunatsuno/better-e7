...
現在は映像preview / ADB入力 / 複数template認識 / 汎用AutomationProfileから入力queueまでの実行経路 / profile再読み込み / dry-run / ゲーム拡張APIまで実装しています。Rustとeguiを使い、Androidとの接続はADBと固定versionのscrcpy-serverを使います。
